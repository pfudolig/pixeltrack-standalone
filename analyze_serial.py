...
def storeByStream(nStreams,maxEvents,socket):
    ...
    for i in range(1,nStreams+1):
        ...
        for j in range(4):
            if socket > 2:
                cmd = "./serial --numberOfThreads " + str(i) + " --numberOfStreams " + str(i) + " --maxEvents " + str(maxEvents)
            if socket == 1:
    ...
    csv_title = serialpath + 'csv/4serial_pin' + str(socket) + '_' + str(nStreams) + 's_' + str(maxEvents) + 'e.csv'
    df.to_csv(csv_title)
