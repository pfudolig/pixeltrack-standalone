...
def storeByStream(nStreams,maxEvents,socket,nThreads):
    ...
    for i in range(1,nStreams+1):
        ...
        for j in range(4):
            if socket > 2:
                cmd = "./serial --numberOfThreads " + str(nThreads) + " --numberOfStreams " + str(i) + ' --maxEvents ' + str(maxEvents)
            if socket == 1:
                cmd = "numactl -N 0 ./serial --numberOfThreads " + str(i) + " --numberOfStreams " + str(i) + ' --maxEvents ' + str(maxEvents)
            if socket == 2:
                cmd = "numactl -N 1 ./serial --numberOfThreads " + str(i) + " --numberOfStreams " + str(i) + ' --maxEvents ' + str(maxEvents)
    ...
    csv_title = serialpath + 'csv/4serial_pin' + str(socket) + '_' + str(maxEvents) + 'e.csv'
    df.to_csv(csv_title)
...
user_output = storeByStream(nStreams,maxEvents,socket,nThreads)