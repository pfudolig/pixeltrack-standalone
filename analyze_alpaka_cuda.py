...
def plotThroughput(dataframe,std='NA'):
    ...
    if std == 'NA':
        ...
        plt.plot(df_streams,df_throughput,'o',linestyle='solid',label='Alpaka Cuda')
        ...
        plt.show() 
        plt.savefig(alpakapath + 'alpcthru_' + str(max(df_streams)) + 's_' + str(events_val) + 'e.png')
        plt.close()

    if std == 'std':
        plt.style.use(hep.style.CMS)
        plt.figure(figsize = (20,10))
        plt.plot(streams_pick,ave_pick,'ro-',linestyle='solid',label='Alpaka Cuda')
        plt.legend(loc='lower right',fontsize='xx-small',frameon=True,shadow=True)
        ...
        plt.show() 
        plt.savefig(alpakapath + '4ave_alpcthru_' + str(max(df_streams)) + 's_' + str(events_val) + 'e.png')
        plt.close()
